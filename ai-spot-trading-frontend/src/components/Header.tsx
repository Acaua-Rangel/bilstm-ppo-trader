...
export const Header = () => {
  ...
  const handleAuthAction = () => {
    if (isAuthenticated) {
      ...
      // Mocked Binance Auth Flow
      navigate('/auth/callback?code=mock_binance_code');
    }
  };

  return (
    <header ref={headerRef} className="fixed top-0 left-0 w-full z-50 glass-card rounded-none border-t-0 border-x-0 bg-background/80">
            ...
            <div className="btn-inner">
              {isAuthenticated ? (
                ...
                <>
                  ...
                  <span className="font-semibold text-white">Sign in with Binance</span>
                </>
              )}
            </div>
          </div>
